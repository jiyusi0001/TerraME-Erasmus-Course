...
function get_world_temp(absorbed_luminosity,luminosity)
	return 200 * absorbed_luminosity*luminosity - 80
end
...
t = Timer{
Event {time = 0, period = 1, action = function()
	
	world_albedo = get_world_albedo(landcover,albedo)
	print(world_albedo)
	
	absorbed_luminosity = 1-world_albedo
	world_temp = get_world_temp(absorbed_luminosity,albedo.luminosity)
	print(world_temp)
	...
	black_daisy_growth_rate = landcover.open_land*get_growth(white_daisy_temp)
	print(black_daisy_growth_rate)

	white_daisy_growth_rate = landcover.open_land*get_growth(black_daisy_temp)
