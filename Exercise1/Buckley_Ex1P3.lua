...
energy_consumption_delta_quarterly = 1+(0.025/4)
energy_consumption_delta_monthly = 1+(0.025/12)
...
t = Timer {
    ...
    Event {time = 3.1, period = 12, action = function()
        if world.dam_volume + rainy_season_one_volume >= dam_max_volume then
            world.dam_volume = dam_max_volume
        else
            world.dam_volume = world.dam_volume + rainy_season_one_volume
        end
    end},
    ...
    Event {time = 8.1, period = 12, action = function()
        if world.dam_volume + rainy_season_two_volume >= dam_max_volume then
            world.dam_volume = dam_max_volume
        else
            world.dam_volume = world.dam_volume + rainy_season_two_volume
        end
    end},
